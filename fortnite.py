...
print(color.CYAN + f'   ███╔══██╗██║   ██║   ██║          ')
print(color.CYAN + f'   ███████╔╝╚██████╔╝   ██║          ')
print(color.CYAN + f'       creato da lorenzo fra2007     ')
print(f'  ')
...
with open('config.json') as f:
    print(f' [iscriviti] [{getTime()}] caricamento....')
    data = json.load(f)
    print(f' [iscriviti] [{getTime()}] caricamento....')
...
async def event_ready():
    print(Fore.GREEN + ' [fatto] [' + getTime() + 'in lobby {0.user.display_name}.'.format(client))
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        if invite.sender.display_name not in data['BlockList']:
            try:
                await invite.accept()
                print(Fore.GREEN + f' [ok] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
            except Exception as e:
                pass
        elif invite.sender.display_name in data['BlockList']:
            print(Fore.GREEN + f' [ok] [{getTime()}] Never accepted party invite from' + Fore.RED + f' {invite.sender.display_name}')
    if data['joinoninvite'].lower() == 'false':
        if invite.sender.display_name in data['FullAccess']:
            await invite.accept()
            print(Fore.GREEN + f' [ok] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
        else:
            print(Fore.GREEN + f' [ok] [{getTime()}] Never accepted party invite from {invite.sender.display_name}')
